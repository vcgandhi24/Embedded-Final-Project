...
ARCHITECTURE logic OF pmod_keypad2 IS
...
BEGIN
  ...
  BEGIN
    IF(clk'EVENT AND clk = '1') THEN  --rising edge of system clock
      rows_int <= rows2;                 --synchronize the row inputs to the system clock
    END IF;
  END PROCESS;
  ...
  BEGIN
    IF(reset_n = '0') THEN                  --reset
      columns2 <= (OTHERS => '1');             --release column outputs
  ...
  row_debounce: FOR i IN 0 TO 15 GENERATE
    debounce_keys: debounce
      ...
      PORT MAP(clk => clk, reset_n => reset_n, button => keys_stored(i), result => keys2(i));
  END GENERATE;
